        ...
        <div className='mealsShortCardsList'>
          <div>
            {meals.filter(meal => meal.type === this.state.mealFilter)
              .map(filteredMeal => (
                <div className='mealsShortCardOne' onClick={() => this.setState({ selectedMeal: filteredMeal })}>
                  <MealCardShort key={filteredMeal.id} meal={filteredMeal} onAdd={this.props.onAdd}/>
